...
function Page({ data }) {
    ...
    return (
        <>
        <Head>
        <title>25cineframes - {data[0].title.rendered}</title>
        <meta charset="utf-8" />
        ...
        <meta property="og:image" content={data[0]['_embedded']['wp:featuredmedia'][0].media_details.sizes.full['source_url'] || data[0]['_embedded']['wp:featuredmedia'][0].media_details.sizes.medium['source_url']} />
        ...
        <meta property="twitter:image" content={data[0]['_embedded']['wp:featuredmedia'][0].media_details.sizes.full['source_url'] || data[0]['_embedded']['wp:featuredmedia'][0].media_details.sizes.medium['source_url']} />
        ...
        <h1 className="text-center pb-5 text-secondary" dangerouslySetInnerHTML={{__html: data[0].title.rendered}}></h1>
         ...
         src={data[0]['_embedded']['wp:featuredmedia'][0].media_details.sizes.full['source_url'] || data[0]['_embedded']['wp:featuredmedia'][0].media_details.sizes.medium['source_url']}
  ...
  export async function getServerSideProps(context) {
    ...
    const response = await fetch(`https://www.25cineframes.com/wp-json/wp/v2/posts?_embed&slug=${slug}`)
